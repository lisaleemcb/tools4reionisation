...
def corner(xs, labels=None, extents=None, truths=None,
    ...
    for i, x in enumerate(xs):
        ax = axes[i, i]
        # Plot the histograms.
        n,b = hist1d( x, smooth=smooth, ax=ax, extents=extents[i], **kwargs)
...
#MD 
    ...
    WW = (ctr_level(z, [0.68,0.95,0.99], infinite=False))[:3]
    WW=WW[::-1]
    #print(W,z.max())
    #print(WW)
    ...
    if plot_datapoints:
        ...
        if plot_contours:
            ...
            ax.contourf(x2, y2, z, WW, alpha=.6, cmap=cmap, antialiased=False)
            ax.contour(x2, y2, z, W[:np.size(sigmas)-1],linewidths=1, alpha=.8, cmap=cmap, antialiased=False)
            

           #
            #ax.contour(x2, y2, z,linewidths=1, alpha=.5, cmap=LinearSegmentedColormap.from_list("cmap", ([1] * 3, [1] * 3),N=2), antialiased=False)
...
def hist1d(x, smooth=0.1, normmax=False, **kwargs):
...
#                      color=kwargs.get("color", "k"),lw=kwargs.get("lw",1),density=True)
    n, b = np.histogram(x, bins=kwargs.get("bins", 50),density=True)
    ns = nd.gaussian_filter1d(n,smooth)
    if normmax:
        ns /= ns.max()
    ax.plot( (b[:-1]+b[1:])/2., ns,
          color=kwargs.get("color", "k"),lw=kwargs.get("lw",1))
    ax.set_xlim( extents)
